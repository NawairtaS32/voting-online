import {AiOutlineCheck} from "react-icons/ai"

export default function CandidateItem() {
    return(
        <div className="flex flex-row border border-zinc-100 p-5 rounded-md space-x-3 ">
            <div className="w-12 h-12 font-bold text-lg items-center flex justify-center bg-zinc-100 text-center  ">
                1
            </div>

            <div className="w-full">
                <h3 className="text-lg font-bold">
                    Nama Kandidat
                </h3>
                <p className="">
                    Kandidat 1
                </p>
                <div className="flex flex-raw items-center space-x-2">
                    <div className="w-full h-4 bg-zinc-100 rounded-full">
                        <div className=" bg-gradient-to-r from-green-400 via-green-600 to-green-900 h-4 rounded-full" style={{ width: "40%"}}>
                        </div>
                    </div>
                    <p className="text-sm font-bold">
                        40%
                    </p>
                    <div className="flex w-20 h-20 items-center justify-center cursor-pointer bg-zinc-100 ">
                        <AiOutlineCheck className="text-2xl font-extrabold" />
                    </div>
                </div>
            </div>
        </div>
    )
};
